...
/* CodeXStudys Profile Navbar controller — single-batch build */
(function(){
  function showProfile(){
    var targets = document.querySelectorAll(
      '#profile, #profilePage, #profileSection, .profile-page, .profile-section, [data-profile-page]'
    );
    if(targets.length){
      targets.forEach(function(el){ el.hidden = false; el.style.display = ''; });
      document.querySelectorAll('[data-codex-profile-nav] .active').forEach(function(el){el.classList.remove('active');});
    }
    var p = document.querySelector('[data-profile-nav], [data-nav="profile"], #profileNav');
    if(p) p.classList.add('active');
  }
  document.addEventListener('click', function(e){
    var item = e.target.closest('[data-codex-profile], [data-nav="profile"], #profileNav');
    if(item){ e.preventDefault(); showProfile(); }
  });
})();
